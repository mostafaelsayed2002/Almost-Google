...
public class Indexer extends Thread {
    ...
    static Set<Word> dictionary = new HashSet<>(); // my datastructure
    static String currenturl;


    static String[] urls = {"MohamedAboutrika.html", "MohamedSalah.html", "CristianoRonaldo.html", "ElonMusk.html", "JeffBezos.html", "BernardArnault.html", "Google.html", "CNN.html", "RealMadrid.html", "LionelMessi.html"};
    ...
    static org.jsoup.nodes.Document ParseDoc(String fileName) {
        String content = null;
        try {
            content = new String(Files.readAllBytes(Paths.get("/Users/mac/Desktop/Almost-Google/backend/src/main/java/" + fileName)));  //get the content of the file
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    static void InsertUpdate(String[] words, int i) {
        ...
        for (String word : words) {  /* For each word int this tag,insert it in the data structure */
            ...
            if (!stopWords.contains(word.toLowerCase())) {  /* First check if this word is stop word */
                ...
                } else {  /* if not already inserted then insert it */
                    Website web = new Website();
                    web.url = currenturl;
                    web.places.add(new Pair<String,String>(type[i],String.join(" ",words)));
                    web.TF++;
    ...
    public static void main(String[] args) {
        ...
        for (String url : urls) {   /// === 3
            count = 0;
            doc = ParseDoc(url); //  Get the parsed document
            currenturl = url;        // Set current url needs to update
        ...
        /* After doing the above code now all word ready to inserted to the database */
        var collection = InitDataBase();

        for (Word item : dictionary) /* iterate on all the dictionary */ {
            ArrayList<Document> ArrayofWebsites = new ArrayList<>();

            JSONObject website = new JSONObject(); // url TF  places[2]
            for (int i = 0; i < item.websites.size(); i++) {

                website.put("url", item.websites.get(i).url); //URL
                website.put("TF", item.websites.get(i).TF);  //TF
                website.put("title", item.websites.get(i).title); //title


                JSONArray places = new JSONArray();

                for (int j = 0; j < item.websites.get(i).places.size(); j++) {
                    JSONObject placeobject = new JSONObject();
                    placeobject.put("place", item.websites.get(i).places.get(j).getFirst());
                    placeobject.put("content", item.websites.get(i).places.get(j).getSecond());
                    places.add(placeobject);

                    website.put("places", places);
                    Document temp = Document.parse(String.valueOf(website));
                    ArrayofWebsites.add(temp);
                }
                ...
                try {
                    InsertOneResult result = collection.insertOne(new org.bson.Document()
                            .append("word", item.word)
                            .append("websites", ArrayofWebsites));
                } catch (MongoException EX) {
                    System.err.println("Unable to insert due to an error: " + EX);
                }
