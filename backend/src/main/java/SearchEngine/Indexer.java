...
public class Indexer {
    ...
    static String currenturl = "https://en.wikipedia.org/wiki/Mohamed_Salah";
    ...
    public static void main(String[] args) {
        ...
        String fileName = "/Users/mac/Desktop/Almost-Google/backend/mostafa.html";
        String content = null;
        try {
            content = new String(Files.readAllBytes(Paths.get(fileName)));  //get the content of the file
        } catch (Exception e) {
            e.printStackTrace();
        }
        org.jsoup.nodes.Document doc = Jsoup.parse(content);  //  parse the content
        //================================================================================


        //============================Select the words in all positions ===================
        String[] type = {"title", "p","h1", "h2", "h3", "h4", "h5", "h6"};
        String[] words;

        for (int i = 0; i < type.length; i++) {   /* Loop on all Tags */
            Elements tags = doc.select(type[i]);   /* Get all the tags of this type */
        ...
        for (Word item : dictionary) /* iterate on all the dictionary */
        {
            ...
            for (int i = 0; i <item.websites.size(); i++) {
                ...
                JSONArray places = new JSONArray();  //places[2]
                for(int j=0;j<item.websites.get(i).places.length;j++)
                  places.put(item.websites.get(i).places[j]);
